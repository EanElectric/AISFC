...
void loop() {
	...
	/*~~GPS Actions~~*/
	if (AISFCGPS::initGPS(AISFC_gps)) {                                   // <- Check if GPS was activated
		if (AISFCGPS::updateGPS(AISFC_gps, lat_mdeg, long_mdeg, gnss_alt))  // <- if GPS has a fresh update, re-calculate distance and bearing from launch. [Leon]: If GPS was not active then updateGPS() would restart the whole sketch
		{
...
bool activateHardware() {
	...
	for (int i = 0; i < 4; i++)  //i < n, where n = number of components used
	{
		...
		case 3:
          ...
          if(i == 30)
          {
            Serial.println("GPS is failing to lock on, find better sky!");
            return false;
          }
				}

				AISFCGPS::zeroLaunchSiteGPS(AISFC_gps, launch_Lat, launch_Long);
