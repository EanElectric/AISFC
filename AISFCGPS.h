...
void getTimeGPS(SFE_UBLOX_GNSS AISFCgps, uint16_t& gps_Year, uint8_t& gps_Month, uint8_t& gps_Day, uint8_t& gps_Hour, uint8_t& gps_Minute, uint8_t& gps_Second)
{
	gps_Year = AISFCgps.getYear();
	gps_Month = AISFCgps.getMonth();
	gps_Day = AISFCgps.getDay();
	gps_Hour = AISFCgps.getHour();
	gps_Minute = AISFCgps.getMinute();
	gps_Second = AISFCgps.getSecond();
}
//
//
